...
class ContactForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not email and not phone:
            raise forms.ValidationError('É necessário fornecer pelo menos um contato: e-mail ou telefone.')
    
    def clean_first_name(self):
        first_name = self.cleaned_data.get('first_name')
        
        if len(first_name) < 2:
            self.add_error('first_name', 'O nome deve conter pelo menos 2 caracteres.')
        return first_name
