...
class ContactForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        contact_id = self.instance.pk

        if first_name and last_name and first_name == last_name:
            self.add_error(
                'last_name',
                ValidationError(
                    'O sobrenome deve ser diferente do nome.',
                    code='invalid',
                )
            )
        ...
        if last_name and (len(last_name) < 3 or len(last_name) > 50):
            self.add_error(
                'last_name',
                ValidationError(
                    'O sobrenome deve conter entre 3 e 50 caracteres.',
                    code='invalid',
                )
            )
        ...
        if phone and Contact.objects.filter(phone=phone).exclude(pk=contact_id).exists():
            self.add_error(
                'phone',
                ValidationError(
                    'Este telefone ja esta em uso.',
                    code='invalid',
                )
            )

        if phone and not re.match(r'^\(?\d{2}\)?\s?9?\d{4}[-.\s]?\d{4}$', phone):
            self.add_error(
                'phone',
                ValidationError(
